...
export class UsuarioListagemPage implements OnInit {
  ...
  //COMO TA FAZENDO A PARTE DO ALERT TEM QUE TER UM ATRBUTO PARA GUARDAR O ID
  protected idUserRole: string = '';

  //Atributo que serve para abrir ou nao um alerta
  ...
  public alertButtons = [
    ...
    },
    {
      ...
      role: 'confirm',
      handler: () => {
        this.usersService.remover(this.idUserRole).subscribe({
          next: () => {
            this.obterUsuarios();
          },
          error: (e) => {
